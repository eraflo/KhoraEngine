...
4. **Tooling** — confirm the bootstrap tools are available (codegraph MCP for symbol lookup, headroom proxy
   for context compression, rtk for token-optimized commands, `/impeccable` for design). The installer sets
   these up; warn if missing, don't block.
