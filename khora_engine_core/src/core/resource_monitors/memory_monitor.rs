...
use crate::core::monitoring::{
    MemoryMonitor, MemoryReport, MonitoredResourceType, ResourceMonitor, ResourceUsageReport,
};
use crate::memory::{get_currently_allocated_bytes, get_extended_memory_stats};
...
impl MemoryResourceMonitor {
    /// Create a new system memory resource monitor.
...
mod tests {
    ...
    fn memory_monitor_creation() {
        let monitor = MemoryResourceMonitor::new("TestMemory".to_string());
