...
use khora_core::renderer::{
    BackendSelectionConfig, GraphicsDevice, RenderError, RenderObject, RenderSettings, RenderStats,
    RenderSystem, RendererAdapterInfo, ViewInfo,
};
...
pub struct WgpuRenderSystem {
    ...
    gpu_monitor: Option<Arc<GpuMonitor>>, // Keep a private handle to the monitor
    current_width: u32,
    current_height: u32,
    frame_count: u64,
...
impl WgpuRenderSystem {
    ...
    /// Asynchronous initialization logic. This is kept separate from the `init` trait
    /// method to make the async nature explicit.
    async fn initialize(
        &mut self,
        ...
        let selection_config = BackendSelectionConfig::default(); // Use default preferences
        ...
        // Pass the pre-selected instance and adapter to the now-passive context.
        ...
        // --- 3. Initialize Graphics Device and Profiler ---
        ...
        if let Ok(gc_guard) = context_arc.lock() {
            if WgpuTimestampProfiler::feature_available(gc_guard.active_device_features) {
                self.gpu_profiler = WgpuTimestampProfiler::new(&gc_guard.device, &gc_guard.queue);
            } else {
                log::info!("GPU timestamp feature not available; instrumentation disabled.");
            }
        }

        // Create and return the monitors
        ...
        self.gpu_monitor = Some(gpu_monitor); // Keep a private handle for updates
...
impl RenderSystem for WgpuRenderSystem {
    ...
    fn shutdown(&mut self) {
        ...
        if let Some(mut profiler) = self.gpu_profiler.take() {
            if let Some(gc_arc_mutex) = &self.graphics_context_shared {
                if let Ok(gc_guard) = gc_arc_mutex.lock() {
                    profiler.shutdown(gc_guard.device());
                }
            }
            log::info!("WgpuTimestampProfiler shutdown complete.");
        }
    ...
    fn get_adapter_info(&self) -> Option<RendererAdapterInfo> {
        self.graphics_device().get_adapter_info().into()
    }
...
// These traits are required for sharing the RenderSystem across threads if needed.
// The internal use of `Arc<Mutex<...>>` makes this safe.
