...
- **Tests**: 586 passing, 35 ignored, 0 failures · Clippy 0 errors
- **Latest work (2026-05-19)**: WGSL migration finalization — Forward+ shadows + OverlayAgent + lane scaffold + legacy WGSL cleanup
  - **Phase 1 — Shadow agent tests**: `ShadowStrategy::from_strategy_id` made `pub`; new tests in `khora-agents/src/shadow_agent/agent.rs` cover the StrategyId→ShadowStrategy mapping (`LowPower→LowRes`, `HighPerformance/Balanced→Standard`) and `apply_budget` end-to-end (5 new tests).
  - **Phase 2 — Forward+ shadow sampling**: `ForwardPlusLane` now reads `ShadowFrame` from the deck (same pattern as `LitForwardLane:300-313`). Layout: Group 3 = shadow bindings (atlas2D + sampler + cubemap via `shadow_bind_group_layout_entries`), Group 4 = F+ tile/light data + new `shadow_view_projs` storage buffer indexed by light. WGSL imports `khora::shadow::sample_2d::sample_shadow_pcf` and applies it to directional/spot inside `fs_main`. Point-light cube shadows in F+ remain a follow-up — `GpuLight` lacks per-face matrices and the `sample_point_shadow` helper signature is tied to `PointLight`.
  - **Phase 3.1 — OverlayAgent**: new `khora-agents/src/overlay_agent/{mod.rs,agent.rs}`. Owns `LaneKind::Render` lanes for post-main-render overlay/debug passes. Runs in `OUTPUT` after `Renderer` (Hard dependency). Lanes run in parallel rather than one-strategy-selected. Registered in `khora-sdk/src/engine.rs` next to `RenderAgent`/`ShadowAgent`. New `AgentId::Overlay` variant.
  - **Phase 3.2 / 3.3 — EmissiveLane / WireframeLane**: overlay lanes registered under `OverlayAgent`. Pipelines composed/validated through `ShaderRegistry` at init. `execute` is a no-op pending the gating data (`MaterialKind::Emissive` flag for Emissive, a debug-flag for Wireframe) — tracked as follow-ups.
  - **Phase 3.4 — GizmoLane (full impl) + infra cleanup**: `GizmoLane` renders editor/debug line gizmos as an alpha-blended LineList overlay (`LoadOp::Load`, no depth). Data flow is CLAD-clean: the editor publishes lines into a shared `Arc<Mutex<GizmoFrame>>` runtime resource (`SharedGizmoFrame`) during `before_agents`; `OverlayAgent` forwards it via `LaneContext`; `GizmoLane` reads it and records the draw in the OUTPUT phase. **`WgpuRenderSystem::{init_gizmo_pipeline, render_gizmos}` + all gizmo fields deleted from `khora-infra`** — the backend no longer owns a gizmo pipeline, and the editor's `after_agents` no longer drives a separate gizmo pass. The editor is now a pure data producer.
  - **Phase 3.5 — StandardPbrLane (full impl)**: alternative `RenderAgent` strategy (`RenderingStrategy::StandardPbr` ↔ `StrategyId::Custom(1)`). `standard_pbr.wgsl` rewritten to use the shared `khora::std::*` / `khora::lighting::*` / `khora::shadow::*` lib modules — same group(3) lighting+shadow contract as LitForward, Cook-Torrance BRDF (GGX + Smith + Schlick) instead of Blinn-Phong. Full multi-light render path with directional/spot 2D shadows + point-light cube shadows.
  - **Forward+ point-light cube shadows**: `GpuLight._unused` renamed to `shadow_far_plane`. `sample_cube.wgsl` split into `sample_point_shadow_params` (explicit params) + `sample_point_shadow` (PointLight wrapper). `forward_plus.wgsl` now samples cube shadows for point lights — full shadow parity with LitForward.
  - **CLAD lane purity**: all four new lanes hold only persistent state — init/render bodies are private free functions in the module, the `Lane` trait impl dispatches to them. No inherent methods, construction via `Default` (matches the `shadows_lane/algo/` pattern and the agent rule in `.agent/rules.md:88`).
  - **Phase 4 — Legacy WGSL cleanup**: 14 duplicate `.wgsl` files at the root of `crates/khora-lanes/src/render_lane/shaders/` deleted (`egui`, `emissive`, `forward_plus`, `gizmo`, `grid`, `light_culling`, `lit_forward`, `shadow_lib`, `shadow_pass`, `standard_pbr`, `text`, `ui`, `unlit`, `wireframe`). The canonical sources live under `shaders/pipelines/` (entry points) and `shaders/lib/` (reusable modules) and are referenced via `ShaderRegistry` + naga_oil `#import`. No runtime filesystem reads for shaders anywhere.
  - **Remaining TODOs** tracked in [`docs/plans/render-lanes-followups.md`](../docs/plans/render-lanes-followups.md): visual validation, EmissiveLane/WireframeLane `execute` bodies (need material/debug flags), real PBR material struct, LitForwardLane CLAD refactor, integration tests, GORNA `Custom` strategy support.
- **Bugfix (2026-05-19) — Forward+ pipeline invalide (5 bind groups)**: la Phase 2 avait fait passer `ForwardPlusLane` à 5 bind groups (camera/model/material/shadow/forward) ; or `wgpu::Limits::default().max_bind_groups == 4` → `create_render_pipeline` produisait une pipeline invalide → `set_pipeline` échouait chaque frame → la scène ne se rendait plus (cube absent, édition + play). Fix : **convention canonique 4 bind groups** documentée dans `.agent/conventions.md` §10 — groupe 3 = domaine *lighting* complet. `ForwardPlusLane` fusionne shadow + tile-data dans le groupe 3 (bindings : `lights@0`, shadow `1/2/3`, `light_indices@4`, `light_grid@5`, `tile_info@6`, `shadow_view_projs@7`) ; `forward_plus.wgsl` repasse ses buffers de `@group(4)` à `@group(3)`. Pas de négociation de limites GPU (resterait sur le flux CLAD). 4 groupes = baseline wgpu universel. Constantes de binding nommées dans `mod g3`.
- **Bugfix (2026-05-19) — grille du viewport effacée → migration en `GridLane`**: la grille de sol était dessinée par `WgpuRenderSystem::render_viewport` dans le hook éditeur `before_agents` (submit immédiat, AVANT le scheduler) ; la passe scène faisait ensuite `LoadOp::Clear` sur la même texture viewport → grille effacée. Bug latent révélé par le fix Forward+ (avant, le submit de la passe scène échouait entièrement → le clear ne s'exécutait pas → grille survivait par accident). Fix : la grille devient `GridLane` sous `OverlayAgent`, rendue après la scène avec `LoadOp::Load` (depth-test `LessEqual` contre la profondeur scène). Opt-in éditeur via `GridConfig` (`khora-data/src/render/grid.rs`) + `SharedGridConfig = Arc<Mutex<GridConfig>>` — l'engine insère le défaut (disabled), l'éditeur l'active dans `setup` ; le sandbox le laisse off (pas de grille). `WgpuRenderSystem::{render_viewport, init_grid_pipeline}` + champs grille supprimés de `khora-infra` ; constante `GRID_WGSL` retirée (`GridLane` passe par `ShaderRegistry`). Le backend ne porte plus aucune pipeline de rendu (grille/gizmo). `OverlayAgent` : ordre `Grid → Emissive → Wireframe → Gizmo`.
- **Older work (2026-05-08, follow-up)**: Save-As policy revert + universal asset tracking + entity→asset prefab drop
