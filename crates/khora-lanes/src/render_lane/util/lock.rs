...
//!
//! Replace `.lock().unwrap()` and `.read()/.write().unwrap()` in render
//! lanes with [`mutex_lock`] / [`read_lock`] / [`write_lock`] so a poisoned
//! lock yields a `LaneError::LockPoisoned` instead of crashing the frame.
...
/// `Result<_, RenderError>` (the `khora_core` GPU error type) — the
