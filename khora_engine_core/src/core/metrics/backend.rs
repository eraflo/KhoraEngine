...
use crate::core::metrics::types::{Metric, MetricId, MetricValue, MetricsError, MetricsResult};
use std::fmt::Debug;
...
pub trait MetricsBackend: Send + Sync + Debug + 'static {
    ...
    fn increment_counter(&self, id: &MetricId, delta: u64) -> MetricsResult<u64> {
        ...
        match metric.value {
            ...
            _ => Err(MetricsError::TypeMismatch {
                expected: crate::core::metrics::types::MetricType::Counter,
    ...
    fn set_gauge(&self, id: &MetricId, value: f64) -> MetricsResult<()> {
        ...
        match metric.value {
            ...
            _ => Err(MetricsError::TypeMismatch {
                expected: crate::core::metrics::types::MetricType::Gauge,
                found: metric.value.metric_type(),
            }),
        }
    }
    ...
    fn record_histogram_sample(&self, id: &MetricId, sample: f64) -> MetricsResult<()> {
        ...
        match metric.value {
            ...
            _ => Err(MetricsError::TypeMismatch {
                expected: crate::core::metrics::types::MetricType::Histogram,
                found: metric.value.metric_type(),
            }),
        }
    }
...
mod tests {
    use super::*;
    use crate::core::metrics::types::{Metric, MetricId};
