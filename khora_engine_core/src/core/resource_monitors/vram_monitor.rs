...
use crate::core::monitoring::{MonitoredResourceType, ResourceMonitor, ResourceUsageReport};
use std::borrow::Cow;
use std::sync::Weak;

/// VRAM Resource Monitor that interfaces with graphics devices to provide
...
/// Trait for providing VRAM usage information
/// This abstraction allows different graphics backends to provide VRAM data
/// without exposing their internal implementation details.
pub trait VramProvider: Send + Sync + std::fmt::Debug {
    /// Get current VRAM usage in megabytes
    fn get_vram_usage_mb(&self) -> f32;

    /// Get peak VRAM usage in megabytes since tracking started
    fn get_vram_peak_mb(&self) -> f32;

    /// Get total VRAM capacity in megabytes (if available)
    fn get_vram_capacity_mb(&self) -> Option<f32>;
}

impl VramResourceMonitor {
    /// Create a new VRAM resource monitor
...
impl ResourceMonitor for VramResourceMonitor {
