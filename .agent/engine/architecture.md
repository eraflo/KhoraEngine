...
| `khora-lanes` | Lanes | Render (Unlit, LitForward, Forward+, StandardPbr, Shadow, Overlay: Grid/Emissive/Wireframe/Gizmo, UI), Physics (Standard, CCD), Audio (SpatialMixing, SourceUpdate), UI (StandardUi, TaffyLayout), `ShaderRegistry`. |
...
WGSL composition: a render lane calls `ShaderRegistry::create_module("khora::pipelines::<name>")` from
`on_initialize`; the registry resolves `#import` via `naga_oil` against `shaders/lib/`, validates with
`naga`, emits final WGSL. No runtime filesystem reads; add a pipeline to `PIPELINE_MODULES` in
`shader_registry.rs`. Detail in [`../../docs/src/06_agents.md`](../../docs/src/06_agents.md) and
[`../../docs/src/08_gorna.md`](../../docs/src/08_gorna.md).
